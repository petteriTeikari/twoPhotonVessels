...
% PETTERI: Use the edges as the penalties
edges = edge(ur,'Canny');

    % re-segment with these penalty points
    [uu2, erriter2,num2,tt2] = CMF_mex(single(edges), single(fCs), single(fCt), single(varParas));
    ...
    subplot(2,2,3)
        ...
        title('Segmentation')
            ...
            % for pruning maybe the edges from small noise
            % see http://stackoverflow.com/questions/1462104/pruning-short-line-segments-from-edge-detector-output
