function [y_est, sigmaEst, PSNR, SSIM] = denoise_BM4D_withAnscombe_wrapper(y)
    ...
    [y, y_sigma, transformLimits] = denoise_anscombeTransform(double(y), 'forward', scaleRange, []);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %       MODIFY BELOW THIS POINT ONLY IF YOU KNOW WHAT YOU ARE DOING       %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    disp('BM4D Volumetric Denoising with Anscombe Transform for Poisson noise')
    ...
    [y_est_VST, sigma_est] = bm4d(y, distribution, (~estimate_sigma)*sigma, profile, do_wiener, verbose);
    sigmaEst = nanmean(sigma_est(:));
    
    % objective result
    ind = y>0;
    PSNR = 10*log10(1/mean((y(ind)-y_est_VST(ind)).^2));
    SSIM = ssim_index3d(y*255,y_est_VST*255,[1 1 1],ind);
    fprintf('Denoising completed: PSNR %.2fdB / SSIM %.2f \n', PSNR, SSIM)

    % inverse Anscombe transform
    [y_est, ~, ~] = denoise_anscombeTransform(y_est_VST, 'inverse', scaleRange, transformLimits);
