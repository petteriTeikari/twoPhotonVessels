function [uu, erriter, num, tt] = test_CMF_2PM_simCs
%
%   Function test_CMF
%
...
ulab1 = [0.0001 0.001 0.01 0.05 0.1 0.15 0.20 0.30 0.50]; % 0.15 default value
ulab2 = 0.6;
...
row = 3; col = 3;
for i = 1 : length(ulab1)
        
    fCs{i} = abs(ur - ulab1(i)); % C_s: point to the capacities of source flows ps
    fCt = abs(ur - ulab2); % C_t: point to the capacities of sink flows pt

    % ----------------------------------------------------------------------
    %  Use the function CMF_Mex to run the algorithm on CPU
    % ----------------------------------------------------------------------
    [uu{i}, erriter{i}, num{i}, tt{i}] = CMF_mex(single(penalty), single(fCs{i}), single(fCt), single(varParas));
