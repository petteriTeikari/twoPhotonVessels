function region = asets_demoWrapper_3D_v3(img, vessel, edges, region, ...
    ...
    for t=1:maxLevelSetIterations
        ...
        % quick'n'dirty fix (PT), boost speed, mid-way
        %{
        if t == ceil(maxLevelSetIterations/2) + 1
            
            largerMask = d_vesselness_outside > d_speed_outside;
            d_speed_outside_old = d_speed_outside;
            addition = d_vesselness_outside;
            addition(~largerMask) = 0;
            d_speed_outside = d_speed_outside + addition;
            
            tau = tau*50;
            disp('quick fix of speed')
            
            diffSum = (d_speed_outside - d_speed_outside_old);
            diffSum = sum(diffSum(:))
            
        end
        %}
        
        % quick'n'dirty "cheat" that seems to be working with leakage, but
        % not sure if this is really a robust way to handle this
        
        if t == ceil(maxLevelSetIterations/2) - 1 && ~secondPass
            
            disp(['  ghettoFix on iter = ', num2str(t)])
            multip = (Cs-Ct);            
            img_n = img_n + (2*multip);
            % img_n(img_n > 1) = 1;
            img_n = img_n / max(img_n(:));            
            
            MinIP_in = min(img_n, [], 3); 
            MaxIP_in = max(img_n, [], 3); 
            set(imH(2), 'CData', MaxIP_in)
            set(imH(3), 'CData', MinIP_in)
            
            %{
            d_speed_outside = d_speed_outside .* multip;
            d_speed_outside = d_speed_outside / max(d_speed_outside(:));
            
            d_speed_inside = d_speed_inside .* (1-multip);
            d_speed_inside = d_speed_inside / max(d_speed_inside(:));
            
            axes(sp(i-1))
                imshow(max(d_speed_outside, [], 3), [])
            axes(sp(i-2))
                imshow(max(d_speed_inside, [], 3), [])
            %}
            
        elseif t == ceil(2*maxLevelSetIterations/3) && ~secondPass
            %{
            multip = (Cs-Ct);
            % img_n = (0.3*multip + 0.7*img_n) .* img_n;            
            img_n = img_n + multip;
            img_n = img_n / max(img_n(:));
            MinIP_in = min(img_n, [], 3); 
            MaxIP_in = max(img_n, [], 3); 
        ...
        elseif t == maxLevelSetIterations - 2
            
            disp(['  ghettoFix on iter = ', num2str(t)])
            multip = (Cs-Ct);            
            img_n = img_n + (3*multip);
            % img_n(img_n > 1) = 1;
            img_n = img_n / max(img_n(:));            
        ...
        if visualizeON
            ...
            fileOut = fullfile('figuresOut', 'segmentation', [fileOutBase, '_', num2str(index), '.png']);
            export_figureToDisk(fig, fileOut, 'png', '-r200', '-a2', options)
